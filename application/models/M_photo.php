...
class M_photo extends CI_Model
{
    ...
    */
    public function add ($title = null, $uid = null, $width=null, $height=null, $size=null, $checksum=null, $category=null)
    {
        if ($title && $uid && $width && $height && $size && $checksum && $category)
        {
            $title = $this->db->escape($title);
            $width = $this->db->escape($width);
            $height = $this->db->escape($height);
            $date = time();
            $category_clean = clean_numeric_text($category);
            $category = (strlen($category_clean) > 0)? $category_clean : 1;
