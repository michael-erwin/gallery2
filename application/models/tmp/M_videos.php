...
class M_videos extends CI_Model
{
    ...
    /**
     * Function 'isPreset' check if file hash of video item already exist in database. Returns
    ...
    {
        $sql = "SELECT `id` FROM `videos` WHERE `checksum`='{$file_hash}' AND `category_id`={$category_id}";
    ...
    */
    public function add ($title = null, $uid = null, $width=null, $height=null, $size=null, $checksum=null, $category=null, $duration=0, $main_visi='public', $sub_visi='public')
    {
        if ($title && $uid && $width && $height)
        {
            ...
            $sql = "INSERT INTO `videos` SET `category_id`={$category}, `title`={$title}, `uid`='{$uid}', `width`={$width}, `height`={$height}, `file_size`={$size}, `checksum`='{$checksum}', `duration`={$duration}, `share_level`='{$visi}', `mc_share_level`='{$main_visi}', `sc_share_level`='{$sub_visi}', date_added={$date}";
