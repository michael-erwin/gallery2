...
class Share extends CI_Controller
{
    ...
    private function share()
    {
        ...
        if($id)
        {
            ...
            if($errors == 0)
            {
                $set_sql   = "UPDATE `videos` SET `share_level`='{$share_level}',`pvt_share_id`='{$pvt_share_id}' WHERE `id` IN({$clean_ids})";
    ...
    {
        $item_sql  = "SELECT `videos`.*,`categories`.`title` AS `sc_title`,`categories`.`parent_id` AS `mc_id` FROM `videos`";
        $item_sql .= " INNER JOIN `categories` ON `videos`.`category_id`=`categories`.`id` WHERE `videos`.`share_level`='private' AND `videos`.`pvt_share_id`='{$share_id}'";
