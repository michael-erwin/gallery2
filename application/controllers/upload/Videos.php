        ...
        if(isset($_FILES['file']) && $category_id)
        {
            ...
            if(move_uploaded_file($_FILES['file']['tmp_name'], $source))
            {
                ...
                else
                {
                    ...
                    if ($id = $this->m_video->add($title,$uid,$dimensions['width'],$dimensions['height'],0,$checksum,$category_id,$duration))
                    {
            ...
            {
                $response['message'] = 'Unable to move temp files.';
            }
